    ...
    rsx! {
        Section { title: "Others",
            div { class: "grid grid-cols-2 gap-4",
                ...
                LocalizationTemplateInput {
                    label: "Lie detector (captcha ready)",
                    template: DetectionTemplate::LieDetectorCaptchaImage,
                    on_value: move |image: Option<Vec<u8>>| async move {
                        save_localization(Localization {
                            lie_detector_captcha_image_base64: to_base64(image, false).await,
                            ..localization()
                        });
                    },
                    value: localization().lie_detector_captcha_image_base64,
                }
            }
        }
    }
